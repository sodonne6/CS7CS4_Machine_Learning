...
MODEL_DIR = "add_sub_model"           # relative to current working dir
os.makedirs(MODEL_DIR, exist_ok=True) # create it if it doesn't exist

MODEL_BASENAME = "mathGPT_add_sub"
...
# dataset lives inside this model dir as a subfolder
TRAIN_PATH = os.path.join(MODEL_DIR, "add_sub_dataset", "maths_add_sub_train.txt")
TEST_PATH  = os.path.join(MODEL_DIR, "add_sub_dataset", "maths_add_sub_test.tsv")
...
def test_model(model,test_rows,max_new_tokens=4):
    ...
    model.train()
    return overall_acc, {"+": add_acc, "-": sub_acc}   
...
if TEST:
    ...
    with open(os.path.join(MODEL_DIR, MODEL_BASENAME + "_test_results.csv"),
              mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["overall_acc", "add_acc", "sub_acc"])
        writer.writerow([overall_acc, op_acc.get("+", 0.0), op_acc.get("-", 0.0)])
